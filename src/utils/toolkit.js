...
export function troncaStringa(str, n = 80) {
  //Se la stringa supera il limite 'n', la taglio e ci metto i tre puntini altrimenti la lascio stare così com'è
  return String(str).length > n ? str.slice(0, n) + "…" : str
}


// Funzione helper per estrarre lat/lng da formato formato WKT tipo POINT(lng lat)
//restituisce un oggetto da usare con Leaflet
export function parsePosizione(stringa_wkt) {
  //console.log('PARSE POSIZIONE')

        // Se la stringa è vuota o nulla, ritorno subito null per evitare errori
        if (!stringa_wkt) {
        ...
        // In caso di errori ritorno comunque dei valori nulli per sicurezza
        return { lat: null, lng: null }
    }