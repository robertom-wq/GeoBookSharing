/**
 * Genera e imposta il token CSRF da inviare tramite cookie csrf (Double Submit Cookie) */
...
export const generateCsrfToken = (req, res) => {
    ...
    if (!token) {
        ...
        res.cookie('csrf_token', token, CSFR_COOKIE_OPTIONS)
    }
    return token
}
...
export const csrf_protection = (req,res,next) => {
    ...
    const header_token = (req.headers['x-csrf-token'] || req.headers['X-CSRF-Token'] || '').trim() //Viene recuperato dall'header HTTP X-CSRF-Token (inviato dal codice JavaScript del frontend)
